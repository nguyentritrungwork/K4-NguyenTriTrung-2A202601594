...
def content_filter(response: str) -> dict:
    ...
    # PII and Secrets patterns to check
    PII_PATTERNS = {
        # Phone numbers: Vietnam mobile/landline numbers (+84, 84, or 0)
        "phone": r"\b(?:\+?84|0)[35789](?:\s*\d){8}\b",
        
        # Emails: Standard email pattern
        "email": r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b",
        
        # National IDs: Vietnamese CMND (9 digits) or CCCD (12 digits)
        "national_id": r"\b\d{9}\b|\b\d{12}\b",
        
        # API Keys: Typical API keys or secrets (e.g. sk-...)
        "api_key": r"\bsk-[a-zA-Z0-9_-]{12,}\b",
        
        # Passwords: English and Vietnamese password assignments
        "password": r"\b(?:password|mật\s*khẩu)\s*(?:is|là|[:=])\s*\S+",
        
        # Database / internal hosts: e.g. db.vinbank.internal:5432
        ...
        *,
        ...
        if self.use_llm_judge:
            ...
            if not safety_res["safe"]:
                ...
                llm_response.content = types.Content(
                    role="model",
                    parts=[types.Part.from_text(text="I cannot provide that information as it violates our security policies.")],
                )
