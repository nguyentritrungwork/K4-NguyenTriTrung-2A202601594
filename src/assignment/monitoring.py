...
class MonitoringAlert:
            ...
            if block_rate > self.block_rate_threshold:
                self.alerts.append(
                    Alert(
                        ...
                        message=f"Block rate exceeded threshold: {block_rate:.2f} > {self.block_rate_threshold:.2f}"
                    )
                )
        ...
        if self.rate_limit_hits > self.rate_limit_hit_threshold:
            self.alerts.append(
                Alert(
                    ...
                    message=f"Rate limit hits exceeded threshold: {self.rate_limit_hits} > {self.rate_limit_hit_threshold}"
                )
            )
            ...
            if judge_fail_rate > self.judge_fail_rate_threshold:
                self.alerts.append(
                    Alert(
                        ...
                        message=f"Judge fail rate exceeded threshold: {judge_fail_rate:.2f} > {self.judge_fail_rate_threshold:.2f}"
                    )
                )
    ...
    def export_json(self, filepath: str = "outputs/metrics.json"):
        ...
        # Ensure metrics are calculated first
        self.check_metrics()
